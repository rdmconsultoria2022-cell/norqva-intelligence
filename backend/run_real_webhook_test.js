...
function fetchRealSupabaseToken() {
...
function callCheckoutAPI(token, payload) {
  return new Promise((resolve, reject) => {
...
async function seedTestEntities(client, sub) {
  ...
  await client.query(
    `INSERT INTO customers (id, name, email, phone, is_demo) 
     VALUES ($1, 'E2E Delivery Customer', 'e2e_del@cust.com', '11987654321', true)`,
    [custId]
  );
...
function callCheckoutPix(orderId, checkoutToken) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: '127.0.0.1',
      port: 5000,
...
function simulateAsaasPayment(providerPaymentId) {
  return new Promise((resolve, reject) => {
    const apiKey = process.env.ASAAS_API_KEY;
    const url = `https://sandbox.asaas.com/api/v3/payments/${providerPaymentId}/simulatePayment`;
...
async function main() {
  ...
  try {
    ...
    console.log('Checkout API response body:', JSON.stringify(orderRes.body));
